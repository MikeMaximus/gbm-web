                    ...
                    <xsl:for-each select="Game">
                      ...
                      <tr>
                          ...
                          <xsl:for-each select="Tags">
                            <xsl:for-each select="Tag">
                              ...
                              <xsl:if test="Name='EGS'">
                                <a>
                                  <xsl:attribute name="href">https://www.epicgames.com/store/en-US/browse?q=<xsl:value-of select="../../Name"/></xsl:attribute>
                        ...
                        <td>                          
                          <ul class="tree">
                          <li>
                            <input type="checkbox"><xsl:attribute name="id"><xsl:value-of select="ID"/></xsl:attribute></input>
                            <label class="tree_label"><xsl:attribute name="title">ID: <xsl:value-of select="ID"/></xsl:attribute><xsl:attribute name="for"><xsl:value-of select="ID"/></xsl:attribute><xsl:value-of select="Name" /> <a class="reportlink" title="Report Broken Configuration"><xsl:attribute name="href">https://github.com/MikeMaximus/gbm-web/issues/new?title=[Broken%20Windows%20Configuration]%20<xsl:call-template name="url-encode"><xsl:with-param name="str" select="Name" /></xsl:call-template>&amp;body=https://mikemaximus.github.io/gbm-web/GBM_Official.xml%23?q=id:<xsl:value-of select="ID"/>%0D%0A%23%23%20The%20following%20is%20not%20working:%0D%0A</xsl:attribute>&#x1F5D2;</a> <a class="permalink" title="permalink"><xsl:attribute name="href">#?q=id:<xsl:value-of select="ID"/></xsl:attribute>&#x1F517;</a></label>                            
                            <ul>
